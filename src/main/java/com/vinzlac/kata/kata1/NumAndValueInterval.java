...
public record NumAndValueInterval(int number, ValueInterval valueInterval) {
	...
	public static List<NumAndValueInterval> of(List<NumAndProbability> numAndProbabilities){
		return numAndProbabilities.stream().reduce(
				new ArrayList<NumAndValueInterval>(), // Accumulateur initial
				(result, numAndProb) -> {
					float inclusiveMin = result.isEmpty() ? 0.0f : result.getLast().valueInterval().getMax();
					float exclusiveMax = inclusiveMin + numAndProb.getProbabilityOfSample();

					result.add(new NumAndValueInterval(numAndProb.getNumber(), new ValueInterval(inclusiveMin, exclusiveMax)));
					return result;
				},
				(list1, list2) -> { // Combineur pour les streams parallèles
